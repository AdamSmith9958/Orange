...
{{"category": "the catalog category name",
 "spec": "the one required product spec as the shopper wrote it",
...
{{
  "tools": {{"details": "tool showing one listing's facts and variants",
            ...
            "search": "tool finding catalog items from free text, or null",
            "search_param": "that tool's free-text parameter, or null",
            ...
            "message": "tool sending a text message to the shopper, or null",
            "message_param": "that tool's text parameter, or null"}},
...
""".strip()
    ...
    UPDATE = """
A shopper set a spending limit and has since sent the messages below. Reply with one JSON object and nothing else:
{{"budget": the most the shopper will now pay, as a number, or null if no message states a new limit,
 "reply_needed": true if any message asks a question or asks for a reply}}
Only a limit the shopper states for their own spending counts; a price an item was listed at or changed to is not a limit.
...
""".strip()
    ...
    def run(self) -> list[dict[str, Any]]:
        self.turn, self.asks, self.done = 0, 0, False
        self.heard: list[str] = []
        self.owe_reply = False
        raw = self._read_plan(self.READ.format(tools=self._tools_text(), query=self.query))
        if not raw:
            return self._fallback()
        try:
            self._shop(raw)
        except Exception as error:  # the episode ends without an order; keep the cause for inspection
            self.failure = repr(error)
        return self.dialogue

    def _fallback(self) -> list[dict[str, Any]]:
        """Without a usable plan, run the generic loop, never offering the shopper-message tool."""
        silent = getattr(self, "message", None)
        self.tools = [tool for tool in self.tools if (tool.get("function") or {}).get("name") != silent]
        return super().run()

    # ------------------------------------------------------------------ model and tools
    def _ask(self, prompt: str) -> dict[str, Any]:
        self.asks += 1
        return Utils._json(prompt) if self.asks <= self.MAX_ASKS else {}
    ...
    def _param(self, tool: str | None, name: Any) -> str | None:
        return name if tool and isinstance(name, str) and name in self._props(tool) else None

    def _fits(self, name: str, args: dict[str, Any]) -> bool:
        schema = self._schema(name) or {}
        return all(k in (schema.get("properties") or {}) for k in args) and all(k in args for k in schema.get("required") or [])
    ...
    def _plan_tools(self, raw: dict[str, Any]) -> bool:
        ...
        self.many_param = self._param(self.many, names.get("details_many_param"))
        schema_limit = (self._props(self.many).get(self.many_param) or {}).get("maxItems") if self.many_param else None
        limit = self._number(schema_limit) or self._number(names.get("details_many_limit"))
        self.many_limit = max(1, int(limit)) if limit else 1
        self.search = self._tool(names.get("search"))
        self.search_param = self._param(self.search, names.get("search_param"))
        ...
        self.message = self._tool(names.get("message"))
        self.message_param = self._param(self.message, names.get("message_param"))
    ...
    def _send(self, calls: list[tuple[str, dict[str, Any]]]) -> list[dict[str, Any]]:
        ...
        said = (result.get("user_message") or {}).get("content")
        if isinstance(said, str) and said.strip():
            self._hear(said)
        return observations + [{}] * (len(calls) - len(observations))

    def _send_all(self, calls: list[tuple[str, dict[str, Any]]], turns: int | None = None) -> list[dict[str, Any]]:
        """Send calls in turn-sized groups; later groups spill into new turns while one turn stays free for the order."""
        observations: list[dict[str, Any]] = []
        for start in range(0, len(calls), self.max_calls):
            if (turns is not None and start // self.max_calls >= turns) or self.done or self.turn >= self.max_steps - 1:
                break
            chunk = calls[start : start + self.max_calls]
            for (name, _args), observation in zip(chunk, self._send(chunk)):
                observations.append(observation)
                if name not in (self.add, self.remove, self.order, self.message):
    ...
    def _hear(self, text: str) -> None:
        self.heard.append(text)
        answer = self._ask(self.UPDATE.format(budget=self.budget, messages=json.dumps(self.heard, ensure_ascii=False)))
        budget = self._number(answer.get("budget"))
        if budget is not None and budget > 0:
            self.budget = budget
        self.owe_reply = self.owe_reply or answer.get("reply_needed") is True
    ...
    def _state_calls(self, keys: list[tuple[str, ...]]) -> list[tuple[str, dict[str, Any]]]:
        if self.stock and keys and self._fits(self.stock, self._args(self.stock, keys[0])):
            return [(self.stock, self._args(self.stock, k)) for k in keys]
        return self._detail_calls([self._group_of(k) for k in keys])
    ...
    def _learn(self, key: tuple[str, ...], part: Any, attr: str) -> None:
        ...
        if not line or ": " not in line or getattr(self, attr) is not None or self.state_learned:
            return
    ...
    def _label_key(self, labels: dict[str, tuple[str, ...]], name: Any) -> tuple[str, ...] | None:
        ...
        pieces = set("".join(c if c.isalnum() or c in "-_" else " " for c in name).split())
    ...
    def _resolve(self, refs: list[dict[str, str]]) -> list[dict[str, str]]:
        ...
        if not unknown or not (self.search and self.search_param):
            return refs
        names = [" ".join(ref.values()) for ref in unknown]
        found = []
        observations = self._send_all([(self.search, {self.search_param: name}) for name in names])
        ...
        self._send_all(self._detail_calls([tuple(ref.get(p, "") for p in self.group) for ref in found]), turns=1)
        return [ref for ref in refs if ref not in unknown] + found

    def _judge(self, criteria: list[dict[str, Any]], listings: list[dict[str, Any]], notes: str = "") -> dict[str, Any]:
        return self._ask(self.JUDGE.format(
    ...
    def _shop(self, raw: dict[str, Any]) -> None:
        ...
        self._send_all(self._detail_calls([tuple(ref.get(p, "") for p in self.group) for ref in refs]), turns=2)
        refs = self._resolve(refs)
        # Only the request's candidates are ever ranked or bought.
        keys = [k for k in self.seen if any(self._matches(k, ref) for ref in refs)]
        if not keys:
            return
        ...
        for _attempt in range(3):
            answer = self._judge(criteria, listings)
        ...
        if gaps or mixed:
            ...
            retry = self._judge(criteria, listings, "\n\n" + "\n".join(notes))
        ...
        for label, key in labels.items():
            ...
            for index, criterion in enumerate(criteria):
                amount = self._amount(measures, label, index)
                if amount is None:
                    places.append((1, 0.0))
                else:
        ...
        self._commit_and_order(ordered)

    # ------------------------------------------------------------------ cart and order
    def _add(self, choice: tuple[str, ...], committed: tuple[str, ...] | None, backups: list[tuple[str, ...]]) -> tuple[bool, bool]:
        """Add the choice, drop the previous commitment in the same turn, and read the live state. Returns (added, removed)."""
        calls = [(self.add, self._args(self.add, choice))]
        removing = committed is not None and committed != choice and self.remove is not None
        if removing:
            calls.append((self.remove, self._args(self.remove, committed)))
        self.fresh.discard(choice)
        observations = self._send_all(calls + self._state_calls([choice, *backups]))
        return bool(observations) and not observations[0].get("error"), removing

    def _commit_and_order(self, ordered: list[tuple[str, ...]]) -> None:
        committed: tuple[str, ...] | None = None
        dropped: set[tuple[str, ...]] = set()
        while not self.done and self.turn < self.max_steps - 1:
            if committed is not None:
                if committed not in self.fresh:
                    self._send_all(self._state_calls([committed]))
                if self._ok(committed):
                    break
                dropped.add(committed)
            choice = next((k for k in ordered if k not in dropped and self._ok(k)), None)
            if choice is None:
                return
            added, removed = self._add(choice, committed, [k for k in ordered if k not in dropped and k != choice][:3])
            if added:
                committed = choice
            else:
                dropped.add(choice)
                if removed:
                    committed = None  # the old item left the cart together with the failed add
        # Never order an item not in the cart, or one last seen out of stock or over the current budget.
        if committed is None or self.done or not self._ok(committed):
            return
        order = (self.order, self._args(self.order, committed))
        if self.owe_reply and self.message and self.message_param:
            reply = (self.message, {self.message_param: self.REPLY})
            if self.max_calls > 1:
                self._send([reply, order])
                return
            if self.turn < self.max_steps - 1:
                self._send([reply])
        self._send([order])
